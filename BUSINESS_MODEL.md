# 💼 F5 — Modelo de Negócio

## Visão: Indústrias no Mercado Digital

**F5** conecta indústrias (manufatureiras, construtoras, etc.) ao mercado digital através de consultoria estratégica + operação completa em marketplaces.

---

## 🎯 O Que Fazemos

### Consultoria Estratégica
- Análise de produto para marketplace
- Otimização de preços e logística
- Estratégia de marketing e visibilidade
- Treinamento de equipes

### Operação em Marketplaces
- Gerenciamento de catálogo
- Integrações com sistemas ERP
- Fulfillment & logistics
- Atendimento ao cliente
- Análise de dados e performance

---

## 💡 Quatro Cenários de Operação

> Para cada cliente, a F5 escolhe **o melhor cenário**. Fonte: alinhamento Raul + Vinicius, jun/2026.

### 1️⃣ BRAÇO DO ONLINE (Mercado Livre / 3P)

**Como funciona**:
- F5 opera a **conta do cliente** no Mercado Livre (e canais 3P)
- Cliente fabrica; F5 vende online em nome da marca dele
- F5 ganha **comissão** sobre as vendas

**Quando usar**:
- Cliente não tem (ou não quer) operação ML interna
- Marca já existe; falta braço operacional digital

**Revenue**: Comissão (% sobre vendas)

---

### 2️⃣ DISTRIBUIDOR / SÓCIO (Operação 100% online)

**Como funciona**:
- F5 e cliente viram **sócios** em estrutura focada **só no digital**
- Nova operação (pode ser nova marca, CNPJ, conta)
- Split de resultado acordado

**Quando usar**:
- Cliente quer crescimento agressivo online
- Aceita sociedade dedicada ao canal digital

**Revenue**: Split da operação (custom)

---

### 3️⃣ COMPRAR E REVENDER

**Como funciona**:
- F5 **compra** do fabricante/fornecedor
- F5 **estoca** e **vende** nos marketplaces
- Margem = preço de venda − custo de compra − operação

**Quando usar**:
- Importação viável (ex.: tapetes China)
- Volume e margem na revenda
- F5 assume capital de giro e estoque

**Revenue**: Margem na revenda

---

### 4️⃣ AMAZON 1P (Venda Direta)

**Como funciona**:
- F5 **cadastra a empresa do cliente** no programa 1P da Amazon
- **Amazon compra direto** do cliente (consignação/venda direta)
- F5 ganha **comissão** pela intermediação, cadastro e operação
- Recebimento típico: **D+60**

**Quando usar**:
- Produto com fit para Amazon Vendor / 1P
- Escala industrial (ex.: **segmento PET** — "cabe 1P tranquilamente")

**Revenue**: Comissão sobre volume 1P

---

### Plataforma F5 (todos os cenários)

Independente do cenário, o cliente recebe:
- KPIs de vendas e performance por produto
- Controle financeiro (NF-e, recebimentos)
- Visibilidade sem operar marketplace

Ver `docs/OPERATING_MODEL.md`.

---

## 💡 Modelos legados (referência)

Os nomes **White Label** e **Co-Creation** mapeiam aproximadamente para:
- White Label ≈ **Cenário 1** (Braço online) ou **4** (1P com marca do cliente)
- Co-Creation ≈ **Cenário 2** (Sócio digital)

---

## 🏪 Presença em Marketplaces

### Plataformas Operadas

| Marketplace | Modelo | Volume | Diferencial |
|-------------|--------|--------|-------------|
| **Amazon** | 1P (Venda Direta) | Alto | D+60 recebimento |
| **Mercado Livre** | 3P (Intermediário) | Alto | D+15 recebimento |
| **Shopee** | 3P/1P | Crescente | Modelo teste |
| **TikTok Shop** | 3P | Emergente | Audience jovem |

### Licenças Diferenciais

#### 1P (Venda Direta)
- F5 é o **vendedor oficial** para Amazon
- Recebimento: **D+60**
- Modelo: Consignação com taxa
- Vantagem: Volume, visibilidade

#### 3P (Intermediário)
- F5 opera como **intermediário autorizado**
- Recebimento: **D+15** (ML), D+30 (Shopee)
- Modelo: Comissão por venda
- Vantagem: Flexibilidade, controle de preço

---

## 💰 Modelo Financeiro

### Fluxo de Receita

```
Vendas Brutas (GMV)
  ↓
Comissões Marketplace (-10% a -20%)
  ↓
Custos Logísticos (-5% a -15%)
  ↓
Custos Operacionais F5 (-5% a -10%)
  ↓
Receita Líquida Partilhada
  ├─ Modelo 1 (White Label): 5% → F5
  └─ Modelo 2 (Conjunto): Split acordado
```

### Exemplo de Operação

**Cenário**: Indústria de eletrodomésticos

**Volume mensal**: R$ 1M de GMV  
**Comissão Marketplace**: -15% = R$ 150K  
**Custos Logísticos**: -10% = R$ 100K  
**Custos Operacionais**: -8% = R$ 80K  

**Receita Líquida**: R$ 670K  

**Split 1 (White Label)**:
- Indústria: R$ 670K - R$ 33.5K = R$ 636.5K
- F5: R$ 33.5K (5%)

**Split 2 (Produto Conjunto - 60/40)**:
- Indústria: R$ 402K
- F5: R$ 268K

---

## 🎯 Estratégia de Crescimento

### Fase 1: PMV (2026)
- Operar 2-3 clientes em 1-2 marketplaces
- Validar modelo White Label
- Refinar operações

### Fase 2: Escala (2026-2027)
- 10-15 clientes em múltiplos marketplaces
- Modelo Produto Conjunto com 3-4 clientes
- Automações operacionais

### Fase 3: Expansão (2027-2028)
- 50+ clientes
- Presença em 4-5 marketplaces
- Próprio marketplace B2B

### Fase 4: Consolidação (2028+)
- Líder em marketplace operations para indústria
- Expansão LATAM
- Eventual IPO/Consolidação

---

## 👥 Segmentação de Clientes

### Primária: Indústria de Manufatura
- Eletrodomésticos
- Produtos eletrônicos
- Ferramenta e hardware
- Artigos esportivos
- Vestuário/Têxtil

### Secundária: Gestoras & Construtoras
- Operações de obra
- Rastreamento
- Coordenação equipes

### Terciária: Distribuidoras
- Produtos de revenda
- Operações multi-marca

---

## 🔐 Diferencial Competitivo

### 1️⃣ Licenças Próprias
- ✅ 1P Amazon (venda direta)
- ✅ 3P Mercado Livre, Shopee (intermediário)
- Resultado: **Flexibilidade máxima**

### 2️⃣ Receebimento Rápido
- D+15 Mercado Livre
- D+60 Amazon (vs D+90 padrão)
- Resultado: **Cash flow melhor**

### 3️⃣ Operação 360
- Não é só plataforma, é **operação completa**
- Consultoria + tech + people
- Resultado: **Risco cliente zero**

### 4️⃣ Flexibilidade de Modelo
- White Label (cliente mantém marca)
- Produto Conjunto (co-criação)
- Resultado: **Opções para cada cliente**

---

## 📊 Métricas de Sucesso

### Para F5
- GMV Total Operado
- Número de Clientes
- Receita Recorrente
- CAC (Customer Acquisition Cost)
- LTV (Lifetime Value)
- Churn Rate

### Para Clientes
- Crescimento de Vendas
- Margem Líquida
- Visibilidade em Marketplace
- Tempo de recebimento
- Suporte operacional

---

## 🚀 Próximos Passos

1. **Validação de Mercado**
   - Entrevistar 10-15 indústrias
   - Testar Modelo 1 (White Label)
   - Medir aceitação

2. **Piloto Técnico**
   - Setup AWS infrastructure
   - Integração com marketplace (API)
   - Integração com ERP cliente

3. **Go-Live Primeiro Cliente**
   - Operação manual (ajustar processos)
   - Documentar playbook
   - Escalar automações

4. **Estrutura Comercial**
   - Definir pricing structure
   - Criar acordo modelo
   - Build sales team

---

**Versão**: 1.0  
**Criado**: 22 de Junho de 2026  
**Status**: Pronto para validação de mercado
